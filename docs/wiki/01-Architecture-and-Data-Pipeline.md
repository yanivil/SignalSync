...
| `watch_only` | the pattern is in `WATCH_ONLY_PATTERNS` (the cup, #109): the row is information, never a buy signal. A breakout of such a pattern keeps `bars_since_break` and its levels but is reported as `WATCHLIST` with the note "breakout listed for information: watch-only pattern" (`demote_watch_only`, applied when the report is written, so the replay keeps measuring the pattern); `meta.watch_only_patterns` lists them |

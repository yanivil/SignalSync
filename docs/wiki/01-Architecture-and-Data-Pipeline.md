...
| `first_listed` | the session of the structure's first `CONFIRMED` report, carried from the previous night's file whatever the row's status in between (a pull-back to the watchlist does not restart the count); `null` for a watchlist row never confirmed |
| `listed_day` | sessions from `first_listed` to `last_date` inclusive, 1 = the first report. A confirmed row past `MAX_LISTED_DAYS` (6 sessions, a Wolfe 5; #115) is retired: listed once under `closed` as `RETIRED` and remembered in the file's `retired` list (ticker, pattern, stop, first_listed, retired_on, detail) for as long as the scan still produces the structure, so it neither returns as new nor sits on the watchlist |
...
| `RETIRED` | a confirmed row listed past `MAX_LISTED_DAYS` (#115), e.g. "listed 7 sessions since 2026-09-02, limit 6"; judged by `carry_listing`, not by the bars |
...
GitHub Actions: daily-scan (01:17 UTC, main only)    GitHub Actions: pages (after each successful daily-scan)
  ...
  commit output/report.md + signals.json              site_charts → charts.json → build_site → _site/ → deploy
                                                      https://yanivil.github.io/SignalSync/
```

* The page is published by the `pages` workflow: it runs when `daily-scan` completes successfully (a push made with the workflow token, like the scan's commit, never triggers other workflows, so the scan's completion is the trigger), on pushes to `main` that touch the page's files, and on demand. It scores the signal log with `tools/evaluate_signals.py`, fetches the recent bars of every ticker on the page with `tools/site_charts.py` (a failure there only leaves the page without charts), builds `_site/` with `tools/build_site.py` and deploys it as a Pages artifact; nothing is committed back. A failed scan leaves the previous build in place, and the page shows the date of the bar it was built from.

* The scan runs on GitHub because the Claude environments sit behind a network policy that blocks market-data hosts; GitHub and PyPI are reachable from both. The repo is public, which also makes GitHub Pages free.
* Exit code 2 (no data) is captured, still committed if the files changed, and then fails the job so the failure is visible.
* The cron is deliberately off the top of the hour: GitHub delays scheduled workflows most at :00 (on 2026-09-05 the 02:00 run started at 06:36 UTC). Delays can still happen; the page states the bar it was built from and says so when that bar is old.
* Delivery was an e-mail until 2026-09-09: a scheduled task in the Claude desktop app fetched the committed report every morning and sent the tables through Gmail. It depended on the desktop app being open and on a language model transcribing the tables, which once dropped a cell, so the page replaced it and the task was retired; its prompt stays on disk under `~/.claude/scheduled-tasks/` should it ever be wanted again.

...
6. Confirmation via `evaluate_breakout` with `neck(j)` as the trigger, from `RS + 1`, floor `low[RS]`: first close above the neckline after RS sets the clock, `age ≤ MAX_BREAKOUT_AGE + 5`, trigger = `neck` at that bar; otherwise watchlist within 5 % of `neck(n−1)` while above the right-shoulder low.
...
5. Confirmation via `evaluate_breakout` with `line13(j)` as the trigger, from `p5 + 1`, floor `low[5]`: first close back above line 1-3 after point 5 sets the clock, `age ≤ MAX_BREAKOUT_AGE + 5`, trigger = the line at that bar; otherwise watchlist within 5 % of `line13(n−1)` while above point 5.

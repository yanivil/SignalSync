...
| Patience (`waited_too_long`) | bars from the pattern's last anchor (handle low, right shoulder, point 5) to the breakout bar, or to today when there is none, may not exceed `MAX_WAIT_BARS` (when set) | a completed base that waits for months is no longer the pattern that was found; before this rule 9 of 17 watchlist rows had been waiting more than 60 sessions, one since May 2025 |

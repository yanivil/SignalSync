...
| `test_patterns.py` | the double bottom (levels recomputed from the notes, single-rule mutations rejected, isolation from the other fixtures, the 200-random-walk false-positive rate); primitives against hand-computed values (pivots and their tie rule, ATR, roundness R², the breakout state table, trend states, volume ratio); every entry/stop/target/risk recomputed independently from the anchors in `notes` and the documented spec-profile formulas (rollback, rim-of-depth, head height, sweet zone, rhythm, volume confirmation); single-rule mutations that must be rejected; legacy-only rules (prior-advance rise, down-trend veto, volume as a bonus) checked under the `legacy` profile; flat/line/random controls; missing bars, zero volume, wick spikes, an unadjusted split; determinism |
| `test_pipeline.py` | retry policy of the per-symbol download (back-off, no final sleep, delisted not retried), universe loading from CSV and the no-source error, the end-to-end mini universe (CSV → download with one throttled and one delisted symbol → alignment → detection → JSON schema, ordering, report formatting incl. the Max buy and Age columns, `--min-score`, exit code 2), the close-out classification of vanished rows, a second run that reports what happened to the first run's rows, the listing rule (`carry_listing`: sessions counted across nights, retirement past the limit, the remembered retirements, the `legacy` profile without it) and a third run that retires a row and then keeps it silent |
| `test_evaluate.py` | `tools/evaluate_signals.py`: the next-open fill (gap and below-stop rows not traded), outcome classification (target / stop / expired / open / no data, R multiples, horizon), summary statistics, and the git signal log built from a temporary repository (de-duplication on first confirmed appearance with the sessions listed, unreadable commits skipped) |
| `test_build_site.py` | `tools/build_site.py`: the grade by day on the list, the view model of a card (levels, chips, action line), the day-on-list lookup against the track record, the track record's statuses and cumulative curve, the rendered page (sections, escaping, badges, the fragment variant), the pivots parsed from the notes, the candlestick chart (bars, levels, a clipped target, pivots) and an empty, stale scan |
| `test_site_charts.py` | `tools/site_charts.py`: the last N sessions per symbol cut at the scan's bar, the JSON written with the offline `yfinance` stand-in, symbols it cannot fetch skipped, a failed download leaving an empty map and a zero exit |
...
| `test_backtest_wolfe_spec.py` | `tools/backtest_wolfe_spec.py`, the 1H Wolfe specification replayed on its own terms: strict three-bar swing points (ties and the last bars excluded), the price and convergence invariants one violation at a time, a deterministic hourly fixture that yields exactly one trade with the specification's levels (entry at the reclaim close, stop half an ATR under point 5, the point-4 high as the target, realised R equal to the projected R), the entry that is never anticipated, the sweep / undercut / window / reclaim-limit reasons, the exit rules (stop before target on one bar, gaps at the open, time exit, open at the end of the data), low-R trades kept and bucketed, one position per symbol, a random-walk ceiling, RSI and ATR equal to `scan`'s, the body / volume / support helpers, the market context read at the last completed session, the bucket tables, the report and the JSON schema, the 730-day intraday clamp, frame cleaning, and `main()` offline through the `yfinance` stand-in |
...
| `backtest` | manual, pushes touching its files | walk-forward replay of the scanner (or the patterns named in `patterns`, e.g. `db` for a candidate not yet in the scan) over the last N sessions; overall / per-pattern / per-score-bucket hit rates and R multiples in the job summary; the run's `backtest.json` is kept as a workflow artifact (`backtest-<run id>`) |
| `backtest-wolfe-spec` | manual, pushes touching its files | replay of the external 1H Wolfe Wave specification (`tools/backtest_wolfe_spec.py`) on the intervals named in `intervals` (default `1h,1d`; hourly bars cover the last 730 days) over `years` of history: the funnel, the win rate and every review factor's buckets in the job summary; the rows and reports are kept as a workflow artifact (`wolfe-spec-<run id>`) |
| `sync-wiki` | pushes to `main` touching `docs/wiki/`, manual | mirror `docs/wiki/` into the GitHub wiki |
| `pages` | after each successful `daily-scan` (`workflow_run`), pushes touching the page's files, manual | score the signal log, fetch the bars behind the charts, build the page, deploy to GitHub Pages at https://yanivil.github.io/SignalSync/ (a dispatch from a branch builds without deploying) |
...
**A specification replayed on its own terms.** `tools/backtest_wolfe_spec.py` is not a variant of the walk-forward replay: it implements an external 1H Bullish Wolfe Wave specification as written (three-bar swings, a 25-session window, a 0.5 ATR stop under point 5, the point-4 high as the target, a time exit, no reward:risk filter) with its own constants block, logs the specification's schema plus the seven factors of the review per trade, and reports the win rate and the outcome per factor bucket. The rules as run, the readings where the specification is silent and the results are on the [tuning page](03-Configuration-and-Tuning.md).

...
| `test_patterns.py` | the experimental double bottom (levels recomputed from the notes, single-rule mutations rejected, isolation from the active detectors and the other fixtures, the 200-random-walk false-positive rate); primitives against hand-computed values (pivots and their tie rule, ATR, roundness R², the breakout state table, trend states, volume ratio); every entry/stop/target/risk recomputed independently from the anchors in `notes` and the documented spec-profile formulas (rollback, rim-of-depth, head height, sweet zone, rhythm, volume confirmation); single-rule mutations that must be rejected; legacy-only rules (prior-advance rise, down-trend veto, volume as a bonus) checked under the `legacy` profile; flat/line/random controls; missing bars, zero volume, wick spikes, an unadjusted split; determinism |
